...
if __name__ == "__main__":
    def asana_grabby():
      ...
      with open('%s_%s_tixdata.csv' % (project_name,run_date), mode='w', newline='') as data_file:
          ...
          completed_by_list = []
          standard_average_close = []
          standard_average_response = []
          low_average_close = []
          low_average_response = []
          urgent_average_close = []
          urgent_average_response = []
          longest_task = 0
          for task in tasks:
              ...
              #try:
                  #urgency_note = ((task['custom_fields'])['enum_value'])['name']
                  #print(task_data)
                  #print("--------------")
                  #time.wait(30)
                  #print(task_notes)
                  ...
                  #time.sleep(60)
                      ...
                      #task_urgency = urgency_note
                      #str(task_urgency)
                  #elif 'Standard Request' in urgency_note:
                      #task_urgency = urgency_note
                      #str(task_urgency)
                  #elif 'Low Priority/Suggestion' in urgency_note:
                      #task_urgency = urgency_note
                      #str(task_urgency)
                  #db = Database(task_data)
                  #for urgency in db['enum_value']:
                      #if urgency['name'] == 'Standard Request':
                          #task_urgency = 'Standard Request'
                      #elif urgency['name'] == 'Low Priority/Suggestion':
                          #task_urgency = 'Low Priority/Suggestion'
                      #elif urgency['name'] == 'Urgent':
                          #task_urgency = 'Low Priority/Suggestion'
              #except KeyError:
                  #task_urgency = int(0)
              #task_urgency = task['custom_fields']
              #task_urgency = task_urgency['name']
              #task_urgency = str(task_urgency).encode('utf-8')
              ...
              if True == task_completed['completed']:
                  ...
                  if 'none' not in completed_date:
                      ...
                      try:
                          task_data2 = (task_data['data'])['custom_fields']
                          #task_data3 = str(task_data2)
                          print(task_data2)

                          #task_data4 = task_data3["name":]
                          #print(task_data4)
                          #time.sleep(15)
                          #task_data5 = task_data4[115:]
                          #print(task_data5)
                          #time.sleep(15)
                          #urgency_note = task_data4[:5]
                          #pprint(urgency_note)
                          #time.sleep(15)
                      ...
                      if 'Normal' in urgency_note:
                          if 'none' not in completed_time:
                              standard_average_close = standard_average_close.append(completed_time)
                              standard_average_response = standard_average_response.append(response_time)
                      if 'LowPri' in urgency_note:
                          if 'none' not in completed_time:
                              low_average_close = low_average_close.append(completed_time)
                              low_average_response = low_average_response.append(response_time)
                      if 'Urgent' in urgency_note:
                          if 'none' not in completed_time:
                              urgent_average_close = urgent_average_close.append(completed_time)
                              urgent_average_response = urgent_average_response.append(response_time)
